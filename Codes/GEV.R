...
### dir for plots
dir.plots.gev = paste0(dir.plots,"/GeV")
...
######################################
  ...
  for(case in 1:4){
    ...
    if(case==1){ Guk[[case]]  = QuantICs2 + theme(legend.position = c(0.8, 0.8),
                                                  legend.background = element_rect(fill="lightgrey", 
                                                  size=0.5, linetype="solid",colour = "black"))}
  }
  ...
  ggarrange(Guk[[1]]+theme(axis.title.x=element_blank(), axis.text.x = element_blank()),
            ...
            Guk[[3]],#+theme(axis.text.y = element_blank()) ,
  ...
  BpQuant10 = ggplot(QuantGG[which(QuantGG$Pr==10),]) +
    ...
    geom_errorbar( aes(x=case, ymin=Qtot_2, ymax=Qtot_9), width=0.2, colour="orange", alpha=0.9, size=1)+
    ylim(c(0,ymax))+
    theme_bw(base_size=15)+
    theme(legend.position = "none",
          axis.title.x=element_blank(),
          axis.text=element_text(size=axis.text.size),
          axis.title=element_text(size=titles.size,face="bold"))+
    scale_fill_manual(name = element_blank(),
                      values = c("#bdd7e7","#6baed6","#3182bd","#08519c"))+
    ylab(label="Discharge [m3/s]")
  ...
  ggarrange(Ggall[[1]],Ggall[[2]],Ggall[[3]],
            labels = c("Q10","Q100","Q1000"),label.y = 0.98,label.x = 0.7,font.label = list(size=20),
            ncol = 3, nrow = 1)
